...
local statusLbl = mkLabel(mainFrame, {Text = "Connecting...", Font = Enum.Font.Code, TextSize = 13, TextColor3 = Color3.fromRGB(160,165,180), TextXAlignment = Enum.TextXAlignment.Center, Size = UDim2.new(1,-40,0,20), Position = UDim2.new(0,20,0,155), ZIndex = 5, TextTransparency = 1})
...
task.delay(0.3, function()
    for _,l in ipairs(mainFrame:GetChildren()) do if l:IsA("TextLabel") then TweenService:Create(l, TweenInfo.new(0.4), {TextTransparency = l.Text == "EVORUSS" and 0 or 0.3}):Play() end end
end)

local startT, lastS = tick(), -1
local msgs = {[0]="Connecting...",[20]="Loading Modules...",[50]="Building 3D Interface...",[80]="Finalizing...",[100]="Complete."}
local pConn; pConn = RunService.Heartbeat:Connect(function()
    local p = math.min((tick()-startT)/6, 1) local pi = math.floor(p*100)
