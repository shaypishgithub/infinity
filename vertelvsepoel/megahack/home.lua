-- ══════════════════════════════════════════════════════════════════
--  home.lua  —  Home tab  |  Glassmorphism Edition
...
return function(deps)
    ...
    -- Лёгкий glassmorphism-фрейм: полупрозрачный bg + stroke белый
    ...
    -- Градиентная полоска сверху карточки (акцент)
    ...
    -- ─── Детектор экзекутора ──────────────────────────────────────────────
    local EXECUTORS = {
        -- { name, check_fn, platform, rating, color }
    ...
    local function detectExecutor()
        ...
        -- Попытка через идентификацию среды
    ...
    return {
        showHome = function(scrollingFrame)
            ...
            -- ── 2. Карточка игрока (glassmorphism) ────────────────────────
            ...
            if detected then
                -- Иконка-кружок с цветом экзекутора
            ...
            for _, link in ipairs(links) do
                ...
                mkButton(row, "Open ↗",
                    ...
                    link.color,
                    function()
                        -- setclipboard доступен в большинстве экзекуторов
