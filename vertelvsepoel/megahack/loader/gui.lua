-- gui.lua · Ultra Glass Minimal Redesign (Premium Edition)
-- Полностью монолитный дизайн, без костылей и косяков со скруглениями.
-- Сохраняет 100% совместимость с вашими logic.lua / theme.lua.
return function(deps)
    ...
    -- ГЛОБАЛЬНЫЕ СТИЛИ
    -- ─────────────────────────────────────────
    local CORNER_RADIUS = 16 -- Скругляем всё по-человечески
    ...
    local function mkGlassSheen(parent, zIdx)
        ...
        sh.Parent = parent
        mkCorner(sh, parent:FindFirstChildWhichIsA("UICorner") and parent:FindFirstChildWhichIsA("UICorner").CornerRadius.Offset or CORNER_RADIUS)
    ...
    -- ─────────────────────────────────────────
    -- MAIN FRAME (Идеальное монолитное стекло)
    -- ─────────────────────────────────────────
    ...
    -- Мягкое неоновое стеклянное обрамление
    local outerStroke = mkStroke(mainFrame, 1.2, Color3.new(1, 1, 1), 0.75)
    mkGlassSheen(mainFrame, 3)

    -- Тонкий нижний неоновый акцент
    ...
    -- ─────────────────────────────────────────
    -- HEADER (Интегрированный, без швов)
    -- ─────────────────────────────────────────
    local headerFrame = Instance.new("Frame")
    headerFrame.Name = "HeaderFrame"
    headerFrame.BackgroundTransparency = 1 -- Полностью прозрачный фон, чтобы не ломать углы основного фрейма
    headerFrame.Size = UDim2.new(1, 0, 0, 50)
    headerFrame.ZIndex = 5
    headerFrame.Parent = mainFrame

    -- Горизонтальный разделитель (аккуратная матовая линия)
    ...
    -- Кнопка закрытия (аккуратный круг)
    ...
    -- ─────────────────────────────────────────
    -- SIDEBAR (Парящий сайдбар, сохраняющий углы)
    -- ─────────────────────────────────────────
    local sidebarFrame = Instance.new("Frame")
    sidebarFrame.Name = "SidebarFrame"
    sidebarFrame.BackgroundTransparency = 1 -- Никаких наложений слоёв на углы
    sidebarFrame.Size = UDim2.new(0, 140, 1, -50)
    sidebarFrame.Position = UDim2.new(0, 0, 0, 50)
    sidebarFrame.ZIndex = 3
    sidebarFrame.Parent = mainFrame

    -- Красивый вертикальный разделитель
    ...
    catScroll.ScrollBarThickness = 0 -- Минимализм: убираем полосу прокрутки для категорий
    catScroll.ZIndex = 5
    ...
    -- Dummy-заглушки для обратной совместимости с logic.lua (теперь они не нужны, но поля требуются)
    ...
    local function createButton(text, parent, callback, isCategoryButton)
        if isCategoryButton then
            ...
            -- ── Стеклянная карточка-кнопка в контенте ──
            local btn = Instance.new("TextButton")
    ...
    return {
        ...
        headerPatch = dummyPatch, -- Возвращаем невидимую пустышку для совместимости с логикой
