-- ══════════════════════════════════════════════════════════════════
--  maybemenu.lua  —  Main Loader  v2 (RussElite Glass Edition)
...
if not playerGui then warn("[RussElite] PlayerGui not found!") return end
...
local function safeLoad(url)
    ...
    warn("[RussElite] failed to load: " .. tostring(url) .. " | " .. tostring(res))
    return nil
end
...
if not baseConfig or not baseConfig.categories then
    warn("[RussElite] base.lua failed or damaged!")
    return
end
...
print("[RussElite] base.lua loaded. Categories: " .. #catNames .. " → " .. table.concat(catNames, ", "))
    ...
    for categoryName, fileName in pairs(categoryMap) do
        local t = task.spawn(function()
            ...
            if type(data) == "table" and #data > 0 then
                ...
                print("[RussElite] ✓ " .. categoryName .. " (" .. #data .. " scripts)")
            else
                warn("[RussElite] ✗ " .. categoryName)
            end
        end)
...
if type(themeFactory) ~= "function" then
    warn("[RussElite] theme.lua failed or wrong format"); return
end
...
if type(guiFactory) ~= "function" then
    warn("[RussElite] gui.lua failed or wrong format"); return
end
...
--  ОБНОВЛЕНО: Добавлен оптимизированный Glass Gradient
-- ══════════════════════════════════════
createNotification = function(title, subtitle, duration, iconId)
    local notifGui = Instance.new("ScreenGui")
    notifGui.Name         = "RE_Notification"
    ...
    bgCorner.CornerRadius = UDim.new(0,12) -- Чуть больше скругление для современного стиля
    ...
    -- Быстрый Glassmorphism эффект (без лишних фреймов)
    local glassGrad = Instance.new("UIGradient", bg)
    ...
    -- Fade in (Адаптировано под новую прозрачность стекла)
    TweenService:Create(bg,       ti_in, {BackgroundTransparency=0.04}):Play()
    TweenService:Create(stroke,   ti_in, {Transparency=0.50}):Play()
    TweenService:Create(bar,      ti_in, {BackgroundTransparency=0.2}):Play()
...
if gui.setNotification then gui.setNotification(createNotification) end
...
if type(logicFactory) ~= "function" then
    warn("[RussElite] logic.lua failed or wrong format"); return
end
