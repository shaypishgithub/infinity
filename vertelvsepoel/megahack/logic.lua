═══════════════════════════════════════════════════════════════
--  logic.lua — Full Logic Layer v3 (COMPLETE)
--  2026 Edition: 3D Glass Neon
═══════════════════════════════════════════════════════════════
...
return function(deps)
    ...
    -- ═══ LOAD SUBMODULES ═══
    local HomeModule, GamesModule, ColorPickerModule, StatsModule
    ...
    local function loadSubmodule(name)
        ...
        if not ok or type(result) ~= "function" then
            warn("[MegaHack] Failed to load submodule: " .. name)
            return nil
        end
        return result
    end
    ...
    if not HomeModule then
        HomeModule = { showHome = function() end }
    end

    -- ═══ INIT GAMES ═══
    GamesModule = loadSubmodule("games.lua")
    if not GamesModule then
        GamesModule = { showGames = function() end, reset = function() end }
    end
    ...
    local function loadColorSettings()
        pcall(function()
            ...
            if data.rgbStroke   ~= nil then settings.rgbStroke   = settings.rgbStroke  end
        end)
    end

    -- Load saved colors before building UI
    loadColorSettings()

    -- ═══ THEME UPDATE ═══
    local theme = deps.theme
    if theme and theme.loadColorSettings then
        theme.loadColorSettings(settings)
    end
    if theme and theme.updateGuiColors then
    ...
    local function createCatButton(name, isSpecial, order)
        ...
        btn.MouseEnter:Connect(function()
            if currentTab ~= name and currentCategory ~= name then
                TweenService:Create(btn, TweenInfo.new(0.15), { BackgroundTransparency = 0.12 }):Play()
                TweenService:Create(hoverStroke, TweenInfo.new(0.15), { Transparency = 0.5 }):Play()
            end
        end)
        ...
        catButtons[name] = {
            ...
            indicator   = indicator,
            textLabel   = btn, -- Reference for text color change
        }
        return btn
    end

    -- Create special tabs
    for i, tab in ipairs(specialTabs) do
        createCatButton(tab.name, true, i)
    end
    ...
    for i, catName in ipairs(sortedCats) do
        createCatButton(catName, false, #specialTabs + 1 + i)
    end
    ...
    function switchTab(name)
        ...
        if StatsModule then
            StatsModule.recordTabClick(name)
        end
    end
    ...
    function switchToCategory(name)
        ...
        if not scripts or #scripts == 0 then
            createLabel("No scripts loaded yet. Check your connection.", scrollingFrame)
            return
        end
            ...
            createButton(scriptName, scrollingFrame, function()
                ...
                pcall(function()
                    ...
                    if fn then
                        ...
                        createNotification("ERROR", "Failed to compile script.", 3)
                    end
                end)
            end)
        end
    ...
    function buildSettingsTab()
        ...
        transSlider.InputChanged:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then updateTrans(input) end
        end)
        UserInputService.InputChanged:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseMovement then updateTrans(input) end
        end)
        ...
        local cpBtn = createButton("✦  Open Color Picker", scrollingFrame, function()
            ...
            colorPickerFrame.Name = "ColorPickerContainer"
    ...
    closeBtn.MouseButton1Click:Connect(function()
        ...
        task.delay(0.3, function()
            ...
            -- Neon pulse on reopen button
            task.spawn(function()
                while not guiOpen do
                    TweenService:Create(reopenButton, TweenInfo.new(1, Enum.EasingStyle.Sine), {
    ...
    UserInputService.InputBegan:Connect(function(input, gpe)
        ...
        if input.KeyCode == Enum.KeyCode.RightControl or input.KeyCode == Enum.KeyCode.LeftControl then
            if guiOpen then
                ...
                reopenButton.MouseButton1Click:Fire()
            end
        end
    end)
